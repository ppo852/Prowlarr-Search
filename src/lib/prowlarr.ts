...
import type { SearchResult, CategoryType, SubCategoryType } from '../types';
import { CATEGORY_MAPPING, SUBCATEGORY_MAPPING } from '../types';
...
class ProwlarrAPI {
  ...
  async search(query: string, category: CategoryType = 'all', subCategory: SubCategoryType = 'all'): Promise<SearchResult[]> {
    ...
    try {
      ...
      if (category !== 'all') {
        if (subCategory === 'all') {
          // Si "Toutes qualités" est sélectionné, utiliser la catégorie principale
          categoryId = CATEGORY_MAPPING[category][0];
        } else {
          // Sinon, utiliser la sous-catégorie spécifique
          const subCategoryIds = SUBCATEGORY_MAPPING[category][subCategory];
          if (subCategoryIds.length > 0) {
            categoryId = subCategoryIds[0];
          } else {
            categoryId = CATEGORY_MAPPING[category][0];
          }
        }
